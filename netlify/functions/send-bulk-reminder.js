...
async function getOutstandingBalanceRecipients() {
  const [parents, children, registrations, ledgers, paymentLogs, emailLogs] = await Promise.all([
    ...
    supabaseQuery("payment_log", { select: "id,parent_id,created_at,method,notes", filters: "&order=created_at.desc&limit=10000" }),
  ...
  // A payment silences the balance reminder only if it actually reduced the balance.
  // Reg fees, shirt orders, and discounts do not. Note strings are generated by
  // create-checkout.js / stripe-webhook.js — if that wording changes, update here.
  const BALANCE_METHODS = new Set(["stripe", "cash", "check", "zelle", "other"]);
  const NON_BALANCE_NOTES = new Set(["Registration fee", "T-shirt order"]);
  const isBalancePayment = (p) =>
    BALANCE_METHODS.has(p.method) && !NON_BALANCE_NOTES.has(p.notes);
  ...
  const paymentsByParent = {};
  for (const p of paymentLogs) {
    if (!isBalancePayment(p)) continue;
    if (!paymentsByParent[p.parent_id]) paymentsByParent[p.parent_id] = [];
    paymentsByParent[p.parent_id].push(p);
  }
  ...
  for (const ledger of ledgers) {
    const balance = (ledger.total_due_cents || 0) - (ledger.total_paid_cents || 0) - (ledger.forgiven_cents || 0);
