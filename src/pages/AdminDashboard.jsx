...
function FamilyModal({ parent, familyChildren, divisions, registrations, weeks, weekMap, divisionMap, ledger, payments, onClose, onSaveParent, onEditChild, onAddChild, onRegisterChild, onRecordPayment, onClearBalance, onSavePaymentPlan, onToggleEarlyBird, saving, isStaff }) {
  ...
  const balance = Math.max(0, (ledger?.total_due_cents || 0) - (ledger?.total_paid_cents || 0) - forgiven);
...
export default function AdminDashboard({ user, setView, showToast }) {
  ...
  const handleToggleWeekReg = async (wk) => { const isOpen = wk.registration_open !== false; try { await sb.query("division_weeks", { method: "PATCH", body: { registration_open: !isOpen }, filters: `&id=eq.${wk.id}`, headers: { Prefer: "return=minimal" } }); showToast(isOpen ? "Registration closed for this week." : "Registration reopened for this week."); load(); } catch (e) { alert("Error: " + e.message); } };
    ...
    <div style={{ minHeight: "100vh", background: colors.bg }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: 24 }}>
                      ...
                      <div style={{ display: "grid", gap: 6 }}>{divWeeks.map((wk) => {
                        ...
                        return (
                          <div key={wk.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: colors.bg, borderRadius: 8, fontSize: 13 }}>
                            ...
                            {!isStaff && <div style={{ display: "flex", gap: 4, alignItems: "center" }}><button onClick={() => handleToggleWeekReg(wk)} title="Toggle whether parents can register for this week" style={{ ...s.btn("ghost"), padding: "3px 9px", fontSize: 11, fontWeight: 600, color: wk.registration_open !== false ? colors.success : colors.coral, border: `1px solid ${wk.registration_open !== false ? colors.success : colors.coral}` }}>{wk.registration_open !== false ? "Open" : "Closed"}</button><button onClick={() => { setWeekModalDivision(div); setWeekModal(wk); }} style={{ ...s.btn("ghost"), padding: "3px 6px", fontSize: 11 }}>{Icons.edit({ size: 12 })}</button><button onClick={() => handleDeleteWeek(wk)} style={{ ...s.btn("ghost"), padding: "3px 6px", color: colors.coral }}>{Icons.trash({ size: 12, color: colors.coral })}</button></div>}
                          </div>);
                      })}</div>
                    )}
                  </div>
        ...
        {tab === "families" && (() => {
          ...
          const exportFamiliesCSV = () => { const headers = ["Parent", "Email", "Phone", "Address", "Parent 2", "Parent 2 Phone", "ELRC", "Children", ...(!isStaff ? ["Total Due", "Paid", "Scholarship", "Balance", "Status"] : [])]; const rows = [headers]; filteredFamilies.forEach((p) => { const kids = children.filter((c) => c.parent_id === p.id); const ledger = ledgerMap[p.id]; const due = ledger?.total_due_cents || 0; const paid = ledger?.total_paid_cents || 0; const forg = ledger?.forgiven_cents || 0; const balance = Math.max(0, due - paid - forg); const cleared = ledger?.balance_cleared || (forg > 0 && balance <= 0); const status = cleared ? "Scholarship" : balance === 0 && due > 0 ? "Paid" : balance > 0 ? "Unpaid" : "—"; const addr = [p.street_address, p.city, p.state, p.zip].filter(Boolean).join(", ") || p.address || ""; const p2Name = [p.parent2_first_name, p.parent2_last_name].filter(Boolean).join(" "); rows.push([p.full_name || "", p.email || "", p.phone || "", addr, p2Name, p.parent2_phone || "", p.elrc_status ? "Yes" : "No", kids.map((k) => `${k.first_name} ${k.last_name}`).join("; ") || "—", ...(!isStaff ? [`$${(due / 100).toFixed(0)}`, `$${(paid / 100).toFixed(0)}`, forg > 0 ? `$${(forg / 100).toFixed(0)}` : "—", cleared ? "Scholarship" : `$${(balance / 100).toFixed(0)}`, status] : [])]); }); const csv = rows.map((r) => r.map((v) => `"${String(v).replace(/"/g, '""')}"`).join(",")).join("\n"); const blob = new Blob([csv], { type: "text/csv" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `cgi-families-${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url); showToast("Exported!"); };
          return (<div>
            <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <div style={{ ...s.card, padding: 0, overflow: "auto" }}>
              {filteredFamilies.length === 0 ? (<EmptyState icon={Icons.users} title="No families found" sub="Adjust your filters or wait for parents to register." />) : (
                ...
                <tbody>{filteredFamilies.map((p) => { const kids = children.filter((c) => c.parent_id === p.id); const ledger = ledgerMap[p.id]; const due = ledger?.total_due_cents || 0; const paid = ledger?.total_paid_cents || 0; const forg = ledger?.forgiven_cents || 0; const balance = Math.max(0, due - paid - forg); const cleared = ledger?.balance_cleared || (forg > 0 && balance <= 0); return (
