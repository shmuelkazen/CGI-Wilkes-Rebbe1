// ============================================================
// calculateBalance — client-side recalculation (ESM)
// Same logic as netlify/functions/utils/calculateBalance.js
// Used by ParentDashboard to compute balance on the fly
// ============================================================

export function calculateBalance({ children, registrations, divisions, weeks, parent, settings, ledger }) {
  ...
  // Early bird: per-division fixed cents, only on full weeks, before deadline OR locked in
  const earlyBirdDeadline = settings?.early_bird_deadline ? new Date(settings.early_bird_deadline) : null;
  const earlyBirdLocked = ledger?.early_bird_locked === true;
  const applyEarlyBird = earlyBirdLocked || (earlyBirdDeadline && new Date() < earlyBirdDeadline);
